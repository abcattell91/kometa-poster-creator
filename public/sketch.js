...
function posterOverflows(poster) {
    ...
    const cased = (v) => (style.uppercase ? String(v).toUpperCase() : String(v));
    const sizeAt = (i) => {
        if (Array.isArray(poster.lineSizes) && poster.lineSizes.length) {
            return poster.lineSizes[i] ?? style.sizeBig;
        }
        return i === 0 ? style.sizeBig : style.sizeSmall;
    };
    return poster.lines.some((text, i) =>
...
class ElementBuiler {
    ...
    static editor() {
        ...
        document.querySelector('#add-line').onclick = () => {
            // New lines default to the "New line size" slider.
            const size = Number(document.querySelector('#edit-size-big').value);
            document.querySelector('#lines-editor')
                .appendChild(ElementBuiler.lineRow({ text: '', size }));
            ElementBuiler.previewDraft();
        };
    ...
    static draftPoster() {
        const { lines, lineSizes, lineColors } = ElementBuiler.readLines();
        const line0 = lines[0] ?? '';
    ...
    /** Replace the row list. `entries` is top-to-bottom. */
    static writeLines(entries) {
        const list = document.querySelector('#lines-editor');
        list.innerHTML = '';
        for (const entry of entries) list.appendChild(ElementBuiler.lineRow(entry));
    }

    /** Read the rows back, dropping blanks. */
    static readLines() {
        const rows = [...document.querySelectorAll('#lines-editor .line-row')];
        const kept = rows
            .map((row) => ({
        ...
        return {
            lines: kept.map((e) => e.text),
            lineSizes: kept.map((e) => e.size),
            lineColors: kept.map((e) => e.colour)
        };
    }

    /**
     * Turn any poster into editor rows, top to bottom. Legacy two-line posters
     * store the small line second but draw it first, so the order is flipped
     * here to match what is actually on screen.
     */
    static linesFor(poster) {
        const colour = poster.textColor ?? '#ffffff';
        if (Array.isArray(poster.lineSizes) && poster.lineSizes.length) {
            return poster.lines.map((text, i) => ({
                text, size: poster.lineSizes[i] ?? 72,
                colour: poster.lineColors?.[i] ?? colour
            }));
        }
        const big = { text: poster.lines?.[0] ?? '', size: poster.sizeBig ?? 72, colour };
        if ((poster.lines?.length ?? 0) < 2) return [big];
        const small = {
            text: poster.lines[1],
            size: poster.sizeSmall ?? 40,
            colour: poster.smallColorLink === false ? (poster.smallColor ?? colour) : colour
        };
    ...
    static loadForm(i) {
        ...
        editingIndex = i;
        ElementBuiler.writeLines(ElementBuiler.linesFor(poster));
    ...
    static resetForm() {
        ...
        ElementBuiler.writeLines([{ text: '', size: 72 }]);
        document.querySelector('input[name="bg"][value="color"]').checked = true;
        document.querySelector('#clear-image').hidden = true;
        document.querySelector('#wh-results .wh-thumb[data-picked="true"]')
            ?.removeAttribute('data-picked');
